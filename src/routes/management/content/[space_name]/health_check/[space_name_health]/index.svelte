<script lang="ts">
    ...
    async function handleErrorEntryClick(err_entry: ModalData, extra: any) {
        ...
        try {
            await Dmart.retrieve_entry(modalData.resource_type, $params.space_name, $params.subpath.replaceAll("-", "/"), $params.shortname, false, false,false)
            isEntryExist = true
        } catch (error) {
            isEntryExist = false
        }
    ...
    const retrieve_entry = Dmart.retrieve_entry;
</script>
